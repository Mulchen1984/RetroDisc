...
# yt-dlp als eigenständige EXE für Offline-Builds
...
def check_vendor() -> bool:
    ...
    required = ["ffmpeg.exe", "ffprobe.exe", "yt-dlp.exe"]
...
def main():
    ...
    # FFmpeg nur herunterladen, wenn eines der beiden Programme fehlt.
    if not (VENDOR_DIR / "ffmpeg.exe").exists() or not (VENDOR_DIR / "ffprobe.exe").exists():
    ...
    if not (VENDOR_DIR / "yt-dlp.exe").exists():
        print()
        download_with_progress(YTDLP_URL, VENDOR_DIR / "yt-dlp.exe", "yt-dlp")
