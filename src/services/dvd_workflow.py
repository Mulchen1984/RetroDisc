...
def _safe_windows_name(value: str, fallback: str = "RetroDisc_DVD") -> str:
    """Creates a short Windows-safe filename component."""
...
class DVDWorkflow:
    ...
    async def run(
        ...
        # Arbeitsverzeichnis vorbereiten. Eine eindeutige, Windows-sichere
        # Kennung verhindert Kollisionen zwischen parallelen Projekten.
        safe_name = _safe_windows_name(project.title)
        work_dir = self.temp_dir / f"dvd_{safe_name}_{uuid.uuid4().hex[:8]}"
    ...
    async def _eject(self, device: str):
        ...
        try:
            ...
            if proc.returncode != 0:
                raise DiscError(stderr.decode("utf-8", errors="replace")[-500:])
...
class AudioCDWorkflow:
    ...
    async def prepare_tracks(
        ...
        for i, src in enumerate(sources):
            ...
            is_cdda = (
                ...
                and media.audio_streams[0].codec == "pcm_s16le"
            )
    ...
    async def burn(
        ...
        if total_min > 80:
            raise DiscError(f"Audio-CD ist mit {total_min:.1f} Minuten länger als 80 Minuten.")
        ...
        import shutil as sh
        cdrecord = self.disc.cdrecord
        if sh.which(cdrecord) is None:
            raise DiscError("Audio-CD-Brennwerkzeug cdrecord/wodim ist nicht installiert.")
