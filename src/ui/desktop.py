...
class RetroDiscAPI:
    ...
    def __init__(self, window=None):
        ...
        self.disc = DiscTools()
        self.ripper = DiscRipper(ffmpeg=self.ffmpeg, disc_tools=self.disc)
    ...
    def _register_handlers(self):
        async def h_convert(job):
            ...
            result = await self.converter.convert_file(
                ...
                overwrite=job.params.get("overwrite", False), job=job)
        ...
        async def h_dvd(job):
            ...
            project = DVDProject(
                ...
                disc_device=job.params.get("disc_device", "/dev/sr0"),
                burn_speed=job.params.get("burn_speed"),
                verify_after_burn=job.params.get("verify_after_burn", True),
                eject_after_burn=job.params.get("eject_after_burn", True),
        ...
        async def h_rip(job):
            job.on_progress = lambda p, t: self._pcb(job)
            job.output_path = await self.ripper.rip(
                device=job.params["device"], output_path=job.output_path,
                output_format=job.params.get("format", "mkv_h265"), job=job)

        async def h_trim(job):
            job.on_progress = lambda p, t: self._pcb(job)
            job.output_path = await self.ffmpeg.trim(
                job.input_files[0], job.output_path,
                job.params["start"], job.params["end"], job)
        ...
        self.pipeline.register_handler(JobType.RIP_DVD.value,           h_rip)
        self.pipeline.register_handler(JobType.TRIM.value,              h_trim)
    ...
    def open_tool_dialog(self) -> str:
        if not self.window:
            return json.dumps({"error": "Kein Fenster verfügbar"})
        result = self.window.create_file_dialog(
            dialog_type=10, allow_multiple=False,
    ...
    def convert_file(self, input_path: str, preset: str,
        ...
        job = Job(job_type=JobType.CONVERT, input_files=[inp], output_path=out,
                  params={"preset": preset, "overwrite": overwrite,
    ...
    def create_dvd(self, paths_json: str, title: str = "RetroDisc DVD",
                   ...
                   speed: Optional[int] = None, verify: bool = True,
                   eject: bool = True) -> str:
        try:
            paths = [Path(p) for p in json.loads(paths_json)]
        except Exception:
            return json.dumps({"error": "Ungültige Pfad-Liste"})
        if burn and not device:
            return json.dumps({"error": "Kein Brenner ausgewählt"})
        job = Job(job_type=JobType.BURN_DVD, input_files=paths,
                  params={"title": title, "standard": standard, "aspect": aspect,
                          ...
                          "disc_device": device or "/dev/sr0",
                          "burn_speed": speed,
                          "verify_after_burn": verify,
                          "eject_after_burn": eject,
                          "display_name": f"{title} -> {'Brennen' if burn else 'ISO'}"})
        return self._submit(job)

    def get_disc_info(self, device: str) -> str:
        try:
            info = self._run_async(self.disc.get_disc_info(device)).result(timeout=20)
            return json.dumps(info)
        except Exception as e:
            return json.dumps({"error": str(e)})

    def rip_disc(self, device: str, fmt: str = "mkv_h265") -> str:
        ext = {"mp4_h264": ".mp4", "mkv_h265": ".mkv",
               "mkv_copy": ".mkv", "iso": ".iso"}.get(fmt, ".mkv")
        label = device.strip().rstrip("\\/").rstrip(":") or "Disc"
        out = self.settings.directories.output_dir / f"Rip_{label}{ext}"
        job = Job(job_type=JobType.RIP_DVD, input_files=[Path(device)], output_path=out,
                  params={"device": device, "format": fmt,
                          "display_name": f"💿 Rip: {device}"})
        return self._submit(job)
    ...
    def detect_burners(self) -> str:
        ...
        if sys.platform != "win32":
            return json.dumps({"drives": [],
                               "error": "Laufwerkserkennung ist derzeit nur unter Windows verfügbar."})
        try:
            drives = self._run_async(self._detect_optical_drives()).result(timeout=15)
            return json.dumps({"drives": drives})
        except Exception as e:
            return json.dumps({"drives": [], "error": str(e)})

    async def _detect_optical_drives(self) -> list:
        import ctypes
        import string
        DRIVE_CDROM = 5
        letters = [f"{c}:" for c in string.ascii_uppercase
                   if ctypes.windll.kernel32.GetDriveTypeW(f"{c}:\\") == DRIVE_CDROM]
        if not letters:
            return []
        names = await self._query_drive_names()
        drives = []
        for letter in letters:
            name = names.get(letter, "Optisches Laufwerk")
            media = await self.disc.get_disc_info(letter)
            drives.append({
                "letter": letter, "name": name,
                "caps": self._caps_from_name(name), "media": media,
            })
        return drives
    ...
    async def _query_drive_names(self) -> dict:
        try:
            ...
            if isinstance(data, dict):
                data = [data]
            return {d.get("Drive", ""): d.get("Caption", "") for d in data if d.get("Drive")}
        except Exception as e:
            log.warning("Laufwerksnamen konnten nicht gelesen werden", error=str(e))
            return {}

    @staticmethod
    def _caps_from_name(name: str) -> list:
        upper = name.upper()
        caps = []
        if "BD" in upper or "BLU" in upper:
            caps.append("Blu-ray")
        if "DVD" in upper:
            caps.append("DVD")
        if "RW" in upper or "RE" in upper or "RAM" in upper:
            caps.append("Brennfähig")
        if not caps:
            caps.append("CD")
        return caps
    ...
    def trim_video(self, input_path: str, start: float,
        ...
        job = Job(job_type=JobType.TRIM,
                  input_files=[inp], output_path=out,
                  params={"start": start, "end": end,
                          "display_name": f"Trim: {inp.name} [{start:.1f}s-{end:.1f}s]"})
        return self._submit(job)

    def preview_trim(self, input_path: str, start: float, end: float) -> str:
        """Erstellt eine kurze Vorschau des Trim-Bereichs und öffnet sie im Standardplayer."""
        try:
            import subprocess
            import sys
            inp = Path(input_path)
            preview_path = self.settings.directories.temp_dir / f"_preview_{inp.stem}{inp.suffix}"
            self._run_async(
                self.ffmpeg.trim(inp, preview_path, start, end)
            ).result(timeout=30)
            if sys.platform == "win32":
                os.startfile(str(preview_path))  # noqa: S606
            elif sys.platform == "darwin":
